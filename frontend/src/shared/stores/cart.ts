...
const SOURCE_IMAGE_KEY = 'customer_cart_source_image'
...
function loadSourceImagePath(): string {
  try {
    return (uni.getStorageSync(SOURCE_IMAGE_KEY) as string) || ''
  } catch {
    return ''
  }
}

function saveSourceImagePath(path: string) {
  if (!path) {
    uni.removeStorageSync(SOURCE_IMAGE_KEY)
    return
  }
  uni.setStorageSync(SOURCE_IMAGE_KEY, path)
}
...
export const useCartStore = defineStore('cart', {
  state: () => ({
    ...
    sourceImagePath: loadSourceImagePath(),
  }),
  ...
  actions: {
    ...
    setSourceImagePath(path: string) {
      this.sourceImagePath = path.trim()
      saveSourceImagePath(this.sourceImagePath)
    },
