...
export function fetchWorkerTasks(options?: { dateFrom?: string; dateTo?: string }) {
  return request<WorkerTask[]>({
...
export function fetchWorkerPickedTasks(options?: { dateFrom?: string; dateTo?: string }) {
  return request<WorkerTask[]>({
    url: '/api/worker/picked-tasks',
    method: 'GET',
    query: {
      dateFrom: options?.dateFrom,
      dateTo: options?.dateTo,
