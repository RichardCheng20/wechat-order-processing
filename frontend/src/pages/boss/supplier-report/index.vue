    ...
    <scroll-view v-else scroll-y class="body-scroll" :show-scrollbar="false">
      <view class="summary-card">
        <view class="summary-main">
          <text class="summary-label">已付货款</text>
          <text class="summary-total">¥ {{ formatMoney(summary?.paidAmount) }}</text>
        </view>
        <view class="summary-breakdown">
          <view class="breakdown-item">
            <text class="tag tag-count">付款笔数</text>
            <text class="breakdown-value">{{ summary?.paymentCount || 0 }}</text>
          </view>
          <view class="breakdown-item">
            <text class="tag tag-supplier">供应商</text>
            <text class="breakdown-value">{{ summary?.supplierCount || 0 }} 家</text>
          </view>
        </view>
      </view>
      ...
      <view class="table-card">
        <view class="table-head">
          <text class="col-name">供应商</text>
          <text class="col-paid">已付金额</text>
        ...
        <view
          ...
          class="table-row"
          @tap="goSupplierDetail(row)"
        >
          <text class="col-name">{{ row.supplierName }}</text>
          <text class="col-paid">{{ formatMoney(row.paidAmount) }}</text>
          <text class="col-count">{{ row.paymentCount }}</text>
        </view>
        <view v-if="rows.length === 0" class="empty-tip">所选区间暂无采购付款记录</view>
      </view>
    </scroll-view>
...
import {
  fetchBossSupplierReport,
  type BossSupplierReport,
...
const summary = ref<SupplierReportSummary | null>(null)
const rows = ref<SupplierReportRow[]>([])
...
async function loadData() {
  ...
  loading.value = true
  try {
    const data: BossSupplierReport = await fetchBossSupplierReport({
...
function goSupplierDetail(row: SupplierReportRow) {
  if (!row.supplierId) return
  uni.navigateTo({ url: `/pages/boss/suppliers/detail/index?id=${row.supplierId}` })
}
...
.quick-tab.active {
  color: #ff9500;
  font-weight: 700;
  box-shadow: inset 0 -4rpx 0 #ff9500;
}
