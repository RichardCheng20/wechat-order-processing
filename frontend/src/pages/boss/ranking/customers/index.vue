  ...
  <view class="page">
    ...
    <scroll-view v-else scroll-y class="body-scroll" :show-scrollbar="false">
      <view class="table-card">
        ...
        <view v-for="row in rows" :key="`${row.customerId || row.customerName}-${row.rank}`" class="table-row">
          <view class="col-name">
            <text class="name-text">{{ row.customerName }}</text>
...
const rows = ref<CustomerRankingRow[]>([])
...
async function loadData() {
  ...
  loading.value = true
  try {
    const data = await fetchBossCustomerRanking({
