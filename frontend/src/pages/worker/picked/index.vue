<template>
  <view class="page">
    <view class="banner">
      <text class="title">已拣单</text>
      <text class="hint">由你标记已拣单的订单记录</text>
    </view>
    ...
    <view v-else-if="tasks.length === 0" class="empty-wrap">
      <u-empty mode="data" text="所选日期内暂无已拣单" />
    </view>
    ...
    <view v-else class="list">
      ...
      >
        <view class="card-main">
          ...
          <text v-if="item.updatedAt" class="meta muted">拣单时间：{{ formatTime(item.updatedAt) }}</text>
        </view>
        <text class="chevron">›</text>
      </view>
    </view>

    <WorkerTabBar active="picked" />
...
import { fetchWorkerPickedTasks, type WorkerTask } from '@common/api/worker'
...
async function loadTasks() {
  ...
  loading.value = true
  try {
    tasks.value = await fetchWorkerPickedTasks({
...
function statusType(status: string) {
  ...
  if (status === 'PENDING_PRICE' || status === 'PRICED') return 'primary'
